...
public:
  ImageConverter(): it_(nh_)
  {
    ...
    img_dep_sub_ = it_.subscribe("/camera/depth/image_raw", 1, &ImageConverter::image_dep, this);
    cv::namedWindow(window_dep);
  }

  ~ImageConverter()
  {
    cv::destroyWindow(window_rgb);
    cv::destroyWindow(window_dep);
  }
  ...
  void image_dep(const sensor_msgs::ImageConstPtr& msg)
  {
    ...
    cv::imshow(window_blur, blur_img);
    cv::imshow(window_cdep, falseColorMap);
    cv::imshow(window_dep, cv_ptr->image);
    cv::waitKey(3);
  }
};
